...
bool CalibProcessor::detectAndParseChAruco(const cv::Mat &frame)
{
    ...
    cv::Mat filteredCorners;
    cv::Mat filteredIds;
    for (int i = 0; i < currentCharucoIds.total(); i++) {
        auto& curId = currentCharucoIds.at<int>(i); 
        if (curId % 10 == 0) continue;

        filteredIds.push_back(currentCharucoIds.row(i));
        filteredCorners.push_back(currentCharucoCorners.row(i));
    }
    currentCharucoIds = filteredIds; 
    currentCharucoCorners = filteredCorners;
...
bool CalibProcessor::checkLastFrame()
{
    ...
    if(!mCalibData->cameraMatrix.total()) {
        ...
        tmpCamMatrix.at<double>(0,2) = mCalibData->imageSize.height/2.0;
        tmpCamMatrix.at<double>(1,2) = mCalibData->imageSize.width/2.0;
    }
    else
    ...
    mBoardSizeInnerCorners(capParams.boardSizeInnerCorners)
{
    ...
    case ChArUco:
        ...
        detector = cv::makePtr<cv::aruco::CharucoDetector>(cv::aruco::CharucoDetector(*mCharucoBoard, charucoParameters, cv::aruco::DetectorParameters()));
        break;
    case CirclesGrid:
    case AcirclesGrid:
